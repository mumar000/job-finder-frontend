...
class ApiClient {
  ...
  async handleResponse(response) {
    ...
    if (!response.ok) {
      // Extract error message from backend format: { success: false, message: "...", errors: {...} }
      ...
      if (typeof data === 'object' && data !== null) {
        ...
        // Class-validator errors format
        else if (data.errors && Array.isArray(data.errors)) {
          errorMessage = data.errors.map(err => err.constraints ? Object.values(err.constraints).join(', ') : err).join('; ');
        }
        // Single error object
    ...
    // Backend success format: { success: true, data: {...}, message: "...", timestamp: "..." }
    // Return the entire response object so consumers can access data, message, etc.
    return data;
  }
